  ...
  return (
    <div style={{ width:"100%", maxWidth:420, animation:"cardIn 0.3s cubic-bezier(0.34,1.4,0.64,1)" }}>
      ...
      <div style={{ textAlign:"center", marginTop:10 }}>
        <span onClick={onHowToPlay} style={{ fontSize:9, color:C.dim, fontFamily:"Georgia,serif", letterSpacing:0.5, cursor:"pointer", textDecoration:"underline" }}>How to Play</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, color:C.white, fontFamily:"Georgia,serif" }}>
      ...
      <div style={{ maxWidth:560, margin:"0 auto", padding:"16px 20px 20px", display:"flex", flexDirection:"column", alignItems:"center" }}>
        ...
        {/* MASTHEAD */}
        <div style={{ textAlign:"center", marginBottom:screen==="game"?6:16, width:"100%", animation:"fadeUp 0.5s ease" }}>
          {screen==="game" ? (
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
              <div style={{ fontSize:15, fontWeight:700, lineHeight:1, letterSpacing:3 }}>
                <span style={{ color:C.gold }}>BOO</span><span style={{ color:C.cream }}>Hist</span>
              </div>
              <span style={{ fontSize:11, color:C.dim }}>·</span>
              <span style={{ fontSize:11, color:C.muted, fontFamily:"Georgia,serif", letterSpacing:1 }}>{mode==="daily"?"Daily Challenge":"Multi-Player"}</span>
            </div>
          ) : (
        ...
        {screen==="game" && card && (
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", width:"100%", animation:"fadeUp 0.3s ease" }}>
            ...
            {!started ? (
              ...
              <DailyCard card={card} choices={choices} onPick={handlePick} onHint={handleDailyHint} timerPct={timerPct} urgent={urgent} cardNum={cardIdx+1} total={deck.length} hintEliminatedIdx={hintEliminatedIdx} flashWrong={flashWrong} onHowToPlay={()=>setShowInstructions(true)} />
            ) : (
